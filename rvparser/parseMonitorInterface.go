...
//parseMonitorInterface will add an interface to an existing internal function
func (t *pParse) parseMonitorInterface() *ParseError {
	var s string
	...
	for {
		...
		//[enforce] event|bool|byte|word|dword|lword|sint|usint|int|uint|dint|udint|lint|ulint|real|lreal|time|any (name[, name]) [on (name[, name]) - non-event types only]; //(comments)
		...
		//still here? attempt to add I/O
		if err := t.addMonitorIO(fbIndex); err != nil {
			return err
		}
	}
}

//addMonitorIO adds a line of event/data [:= default] to the FB interface
// some error checking is done
//fbIndex: the index of the Function we are working on inside t
func (t *pParse) addMonitorIO(fbIndex int) *ParseError {
	fb := &t.funcs[fbIndex]
	...
	if err := fb.AddIO(intNames, typ, size, initialValue); err != nil {
